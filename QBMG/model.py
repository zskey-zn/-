...
class RNN():
    ...
    def likelihood(self, target):
        ...
        for step in range(seq_length):
            ...
            log_probs += NLLLoss(log_prob, target[:, step])#every word in chemmoemolar'p add
    ...
    def sample(self, batch_size, max_length=150):
        ...
        return sequences.data, log_probs, logits
...
def NLLLoss(inputs, targets):
    ...
    if torch.cuda.is_available():
        target_expanded = torch.zeros(inputs.size()).cuda()#torch.Size([32, 89])
    else:
        target_expanded = torch.zeros(inputs.size())

    target_expanded.scatter_(1, targets.contiguous().view(-1, 1).data, 1.0)
    loss = Variable(target_expanded) * inputs
    loss = torch.sum(loss, 1)#torch.Size([32])
    return loss
